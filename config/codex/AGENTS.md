## Python

### Type hints

Assume Python >=3.12 unless project config says otherwise.

For type hints, prefer built-in generics and PEP 604 unions where available:

- `list` instead of `typing.List` (same for `dict` and `tuple`)
- `str | None` instead of `Optional[str]`

### Docstrings

Very simple, easy-to-understand functions can use a one-liner docstring (e.g. `"""Raise input 'n' to the power of 2."""`)

For more complex functions, you must use the following docstring format which is a derivative of Google-style docstring convention with some minor changes. Docstring types must match annotations. The example of a valid docstring looks like this:

```python
def my_function(input_1: str, input_2: int | None = None) -> tuple[str, dict[str, str]]:
    """
    <Explanation of the function in imperative mood>

    Args:
        > input_1 (str):
            Description of input_1.
        > input_2 (int | None):
            Description of input_2. Defaults to None.
    
    Returns:
        - str: <explanation of the first element in the returned tuple>
        - dict[str, str]: <explanation of the second element in the returned tuple>
    
    Raises:
        - ValueError: When <explanation>
    """
```

*(!)* Note that `> input` style is custom, it's intentional.

---

## Documentation

Use the `→` symbol when documenting any sequence of steps, including software algorithms, workflows, procedures, or everyday examples.

For example:

> Prepare the ingredients → Make the batter → Pre-heat the oven → Pour the batter into the cake tin → Bake for 45 minutes

---

## Git

**Commands**:

- Don't EVER run any command that affect the remote origin!
- Only ever run commands like `git add` or `git commit` when the user EXPLICITLY asks for it!

**Commit messages**:

- Propose optional body for the commit if extra context would be useful to understand the commit
- Whenever code is generated by Codex in a commit-by-commit workflow, propose the corresponding commit message for every commit implemented
- Use conventional commit messages with the format: `docs(frontend): Add linting instructions in README.md`

## What NOT TO DO

Under NO CIRCUMSTANCES ever perform non-read actions that affect anything outside of the scope of the environment you are working in. For example, when it comes to working with Github PRs:

- It's OK to read PR contents, fetch comments, diffs etc.
- It's NOT OK to post comments on my behalf after a review, push the changes or do anything else that goes outside of the scope of reviewing the code locally and writing new code locally.
